...
void meas_init(void) {
    printf("Measurement module initialized (simulation mode)\n");
    printf("ADC: %d samples/sec, Vref=%.0f mV, offset=%.2f V\n",
        (int)FS, VREF_MV, DC_OFFSET_V);
}

void meas_start(void) {
    printf("Measurement started (simulation)\n");
    sample_index = 0;
    buffer_ready = 0;
}
...
void meas_set_signal(float frequency_hz, float amplitude_v) {
    ...
    printf("Signal set: %.2f Hz, %.2f V amplitude\n", signal_freq, signal_amp);
}
